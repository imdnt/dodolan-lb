...
module.exports = Account => {
  ...
  //   next();
  // });
  Account.sendEmail = async (req, res, cb) => {
    // const email = await sendEmail.contact(req.name, req.email, req.message);
    return {
      message: "success",
      status: "success"
    };
  ...
  Account.observe("before save", async (ctx, next) => {
    if (ctx.instance == null) {
    //   if (ctx.data.phoneNumber) {
    ...
    //         type: "REGISTER_PHONE_NUMBER_EXISTS",
    //         model: "ACCOUNT",
    //         detail: err.message
    //       });
    //       next(err);
    ...
    //   const phoneNumber = ctx.instance.phoneNumber.split("");
    //   let newPhoneNumber;
    //   if (phoneNumber[0] == "0") {
    //     newPhoneNumber = "+62" + ctx.instance.phoneNumber.substring(1);
    //     ctx.instance.phoneNumber = newPhoneNumber;
    //   }
      next();
    }
  });

  Account.observe("after save", async (ctx, next) => {
    // const influencer = await app.models.Influencer.findOne({
    //   where: {
    //     accountId: ctx.instance.id
    //   }
    // });
    ...
    //     phoneNumber: ctx.instance.phoneNumber,
    //     fullname: ctx.instance.fullname
    //   });
    // }
  });

  Account.beforeRemote("login", async (ctx, account, next) => {
    // every login using lowercase
    let username;
    let email;
    if (ctx.req.body.username != null) {
    //   if (checkWhiteSpace(ctx.req.body.username)) {
    //     throw errorHelper.badInput("username");
    //   }
    //   username = ctx.req.body.username.toLowerCase();
      ctx.req.body.username = username;
    }
    if (ctx.req.body.email != null) {
    //   if (checkWhiteSpace(ctx.req.body.email)) {
    //     throw errorHelper.badInput("email");
    //   }
      email = ctx.req.body.email.toLowerCase();
      ctx.req.body.email = email;
    }
  ...
  Account.afterRemote("login", async (ctx, accessToken, next) => {
    // console.log('account, : ', account);
    const account = await Account.findById(accessToken.userId);
    try {
      ctx.res.send({
        login: true,
        id: accessToken.id,
  ...
  Account.beforeRemote("logout", async (ctx, accessToken, next) => {
    try {
      if (accessToken) {
        ctx.res.send({
          status: "success",
  ...
  Account.beforeRemote("create", async (ctx, next) => {
    // if (checkWhiteSpace(ctx.req.body.username)) {
    //   throw errorHelper.badInput("username");
    // }
    ...
    if (
      ctx.req.body.userType &&
      ctx.req.body.userType.toLowerCase() !== "brand"
    ) {
      throw errorHelper.badInput("userType");
    }
